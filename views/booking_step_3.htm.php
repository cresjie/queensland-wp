				...
				<form action="<?php echo $quitenicebooking_step_4_url; ?>" class="booking-fields-form clearfix" method="POST">
					...
					<?php foreach ($quitenicebooking_form_fields as $field) { ?>
						<?php if ($field['type'] == 'guest_first_name') { // predefined ?>
							<div class="input-field <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_first_name"><?php _e('First Name', 'quitenicebooking'); ?> *</label>
								<input type="text" name="guest_first_name" id="guest_first_name" class="form-control" value="<?php echo !empty($quitenicebooking_guest_first_name) ? $quitenicebooking_guest_first_name : ''; ?>">
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'guest_last_name') { // predefined ?>
							<div class="input-field <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_last_name"><?php _e('Last Name','quitenicebooking'); ?> *</label>
								<input type="text" name="guest_last_name" id="guest_last_name" class="form-control" value="<?php echo !empty($quitenicebooking_guest_last_name) ? $quitenicebooking_guest_last_name : ''; ?>">
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'guest_email') { // predefined ?>
							<div class="input-field <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_email"><?php _e('Email Address','quitenicebooking'); ?> *</label>
								<input type="text" name="guest_email" id="guest_email" class="form-control" value="<?php echo !empty($quitenicebooking_guest_email) ? $quitenicebooking_guest_email : ''; ?>">
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'text') { // text field ?>
							<div class="input-field <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_details_<?php echo $field['id']; ?>"><?php echo $field['label']; ?><?php echo !empty($field['required']) ? ' *' : ''; ?></label>
								<input type="text" name="guest_details_<?php echo $field['id']; ?>" class="form-control" id="guest_details_<?php echo $field['id']; ?>" value="<?php echo !empty(${'quitenicebooking_guest_details_'.$field['id']}) ? ${'quitenicebooking_guest_details_'.$field['id']} : ''; ?>" <?php echo !empty($field['required']) ? 'data-required="1"' : ''; ?> <?php echo !empty($field['class']) ? 'class="'.$field['class'].'"' : ''; ?> <?php echo !empty($field['maxlength']) ? 'maxlength="'.$field['maxlength'].'"' : ''; ?>>
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'textarea') { // textarea field ?>
							<div class="input-field-full-width clearboth form-group">
								<label class="control-label" for="guest_details_<?php echo $field['id']; ?>"><?php echo $field['label']; ?><?php echo !empty($field['required']) ? ' *' : ''; ?></label>
								<textarea name="guest_details_<?php echo $field['id']; ?>" class="form-control" id="guest_details_<?php echo $field['id']; ?>" rows="10" <?php echo !empty($field['required']) ? 'data-required="1"' : ''; ?> <?php echo !empty($field['class']) ? 'class="'.$field['class'].'"' : ''; ?> <?php echo !empty($field['maxlength']) ? 'maxlength="'.$field['maxlength'].'"' : ''; ?>><?php echo !empty(${'quitenicebooking_guest_details_'.$field['id']}) ? ${'quitenicebooking_guest_details_'.$field['id']} : ''; ?></textarea>
							</div>
							<?php $colcount = $colcount % 2 == 1 ? $colcount : $colcount + 1; ?>
						<?php } elseif ($field['type'] == 'checkbox') { // checkbox field ?>
							<div class="input-field checkbox-group <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_details_<?php echo $field['id']; ?>"><?php echo $field['label']; ?><?php echo !empty($field['required']) ? ' *' : ''; ?></label>
								...
								<?php foreach ($field['choices'] as $choice) { ?>
									<div class="checkbox-wrapper">
										<input type="checkbox" name="guest_details_<?php echo $field['id']; ?>[]"  id="guest_details_<?php echo $field['id']; ?>_<?php echo $c; ?>" value="<?php echo htmlentities($choice); ?>" <?php echo !empty($field['required']) ? 'data-required="1"' : ''; ?>><label class="control-label" for="guest_details_<?php echo $field['id']; ?>_<?php echo $c ++; ?>"><?php echo $choice; ?></label>
									</div>
								<?php } ?>
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'radio') { // radio field ?>
							<div class="input-field checkbox-group <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								...
								<?php foreach ($field['choices'] as $choice) { ?>
									<div class="checkbox-wrapper">
										<input type="radio" name="guest_details_<?php echo $field['id']; ?>" id="guest_details_<?php echo $field['id']; ?>_<?php echo $c; ?>" value="<?php echo htmlentities($choice); ?>" <?php echo !empty($field['required']) ? 'data-required="1"' : ''; ?>><label class="control-label" for="guest_details_<?php echo $field['id']; ?>_<?php echo $c ++; ?>"><?php echo $choice; ?></label>
									</div>
								<?php } ?>
							</div>
							<?php $colcount ++; ?>
						<?php } elseif ($field['type'] == 'select') { // select field ?>
							<div class="input-field select-group select-wrapper <?php echo $colcount % 2 == 0 ? 'last-col' : ''; ?> form-group">
								<label class="control-label" for="guest_details_<?php echo $field['id']; ?>"><?php echo $field['label']; ?><?php echo !empty($field['required']) ? ' *' : ''; ?></label>
								<select class="for-control" name="guest_details_<?php echo $field['id']; ?>[]" id="guest_details_<?php echo $field['id']; ?>" <?php echo !empty($field['multiple']) ? 'multiple' : ''; ?> <?php echo !empty($field['required']) ? 'data-required="1"' : ''; ?>>
									<?php foreach ($field['choices'] as $choice) { ?>
					...
					<div id="services" class="clearfix">
						<div class="input-field checkbox-group form-group">
							<label class="control-label"><?php _e('Services', 'quitenicebooking'); ?></label>
							<?php $service_count = 1; ?>
							<?php foreach ($quitenicebooking_service_choices as $s) { ?>
								<div class="checkbox-wrapper">
									<input type="checkbox" name="services[]" value="<?php echo $s['ID']; ?>" id="services_<?php echo $service_count; ?>" <?php if (!empty($quitenicebooking_services)) { checked(in_array($s['ID'], $quitenicebooking_services) ? 1 : 0, 1); } ?>> <label class="control-label" for="services_<?php echo $service_count; ?>"><?php echo $s['post_title']; ?> (<?php echo $this->settings['currency_unit']; echo Quitenicebooking_Utilities::float_to_user_price($s['quitenicebooking_service_price'], $this->settings); ?>)</label>
								</div>
							<?php $service_count ++; ?>
							<?php } ?>
						</div>
					...
					<?php if (!empty($quitenicebooking_coupons_enabled)) { ?>
						...
						<div class="coupon coupon-field clearfix">
							<input type="text" name="<?php echo "coupon_code_{$couponcount}"; ?>" class="coupon-input form-control" id="<?php echo "coupon_code_{$couponcount}"; ?>">
					...
					<div class="payment_method">
						<?php if (!empty($quitenicebooking_accept_paypal) && !empty($quitenicebooking_paypal_email_address)) { ?>
							<h3><input type="radio" name="payment_method" value="paypal" id="payment_method_paypal"><label class="control-label" for="payment_method_paypal"><?php _e('Paypal', 'quitenicebooking'); ?></label></h3>
							<div>
						...
						<?php if (!empty($quitenicebooking_accept_bank_transfer)) { ?>
							<h3><input type="radio" name="payment_method" value="bank_transfer" id="payment_method_bank_transfer"><label class="control-label" for="payment_method_bank_transfer"><?php _e('Direct bank transfer', 'quitenicebooking'); ?></label></h3>
							<div>
					...
					<input type="submit" class="submit btn btn-primary" name="booking_step_3_submit" value="<?php $accept_deposit ? _e('Book Now &amp; Pay Deposit', 'quitenicebooking') : _e('Book Now', 'quitenicebooking') ?>">
