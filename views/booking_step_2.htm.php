	...
	<div class="row">
		<div class="col-md-4">
			...
			<div class="booking-side-wrapper">
				...
				<!-- BEGIN .booking-side -->
				<div class="booking-side sidewidget-lg clearfix">
					...
					<div class="display-reservation ">
						...
						<a href="#" class="button3 edit-reservation btn btn-primary"><?php _e('Edit Reservation','quitenicebooking'); ?></a>
						...
						<form class="booking-form" action="<?php echo $quitenicebooking_step_2_url; ?>" method="POST">
							...
							<div class="clearfix">
								<div class="one-half-form form-group">
									<label class="control-label" for="datefrom"><?php _e('Check In','quitenicebooking'); ?></label>
									<input type="text" id="datefrom" name="room_<?php echo $quitenicebooking_room_number; ?>_checkin" value="<?php echo ${'quitenicebooking_room_'.$quitenicebooking_room_number.'_checkin'} ?>" class="datepicker form-control">
								</div>
								<div class="one-half-form last-col form-group">
									<label class="control-label" for="dateto"><?php _e('Check Out','quitenicebooking'); ?></label>
									<input type="text" id="dateto" name="room_<?php echo $quitenicebooking_room_number; ?>_checkout" value="<?php echo ${'quitenicebooking_room_'.$quitenicebooking_room_number.'_checkout'} ?>" class="datepicker form-control">
								</div>
							</div>
							...
							<div class="rooms-wrapper">
								...
								<div class="room-1 clearfix">
									...
									<div class="one-third-form form-group">
										<label class="control-label" for="room_<?php echo $quitenicebooking_room_number; ?>_adults"><?php _e('Adults','quitenicebooking'); ?></label>
										<div class="select-wrapper">
											<select class="form-control" name="room_<?php echo $quitenicebooking_room_number; ?>_adults" id="room_<?php echo $quitenicebooking_room_number; ?>_adults">
									...
									<?php if (empty($quitenicebooking_remove_children)) { ?>
										<div class="one-third-form last-col form-group">
											<label class="control-label" for="room_<?php echo $quitenicebooking_room_number; ?>_children"><?php _e('Children','quitenicebooking'); ?></label>
											<div class="select-wrapper">
												<select class="form-control" name="room_<?php echo $quitenicebooking_room_number; ?>_children" id="room_<?php echo $quitenicebooking_room_number; ?>_children">
												<?php foreach (range(0, $quitenicebooking_max_persons_in_form) as $r) { ?>
							...
							<input type="submit" class="bookbutton btn btn-primary" name="edit_reservation" value="<?php _e('Check Availability', 'quitenicebooking'); ?>">
				...
				<?php if ($quitenicebooking_prev_room_number !== NULL) { ?>
					<div class="booking-side clearfix back-wrapper">
						<form action="<?php echo $quitenicebooking_step_2_url; ?>" method="POST">
							<input type="hidden" name="booking_step_2_submit" value="1">
							<button type="submit" class="button3 btn btn-primary" name="room_<?php echo $quitenicebooking_prev_room_number; ?>_selection" value=""><?php _e('&larr; Edit previous room', 'quitenicebooking'); ?></button>
						</form>
					</div>
				<?php } // end if ?>
				<!-- END back-button -->
		...
		<div class="col-md-8">
				...
				<div class="booking-main booking-main-highlight">
					...
					<?php } ?>
						...
						<ul class="room-list-wrapper clearfix list-unstyled">

							<!-- BEGIN .room-item -->
							<li class="room-item clearfix <?php echo !$highlight_found ? 'room-unavailable' : ''; ?>">

								<h3><a href="<?php echo get_permalink($room['id']); ?>" target="_blank"><?php echo $room['title']; ?></a></h3>

								<!-- BEGIN .room-list-left -->
								<div class="room-list-left spacer-bottom-15">
									<?php if (isset($room['_thumbnail_id'])) { ?>
										<?php $thumbnail = wp_get_attachment_image_src($room['_thumbnail_id'], 'image-style3'); ?>
										<a href="<?php echo get_permalink($room['id']); ?>" target="_blank"><img class="img-responsive" alt="<?=$room['title']?>" src="<?php echo $thumbnail[0]; ?>"  /></a>
									<?php } ?>
								<!-- END .room-list-left -->
								</div>
								...
								<div class="room-list-right">
										...
										<?php if ( $highlight_found ) { ?>
											...
											<div id="price_break_hotel_room_<?php echo $room['id']; ?>" class="hide">
												...
												<div class="page-content">
													...
													<table class="table margin-none"><tbody>
									...
									<?php foreach ($quitenicebooking_beds->keys['beds'] as $bed => $defs) { // display "Select <bed>" buttons ?>
										<?php if (isset($room[$defs['meta_key']]) && !empty($room[$defs['meta_key']])) { ?>
											<?php if ( $highlight_found ) { ?>
												<button type="submit" class="button2 btn btn-warning" name="room_<?php echo $quitenicebooking_room_number; ?>_selection" value="type=<?php echo $room['id']; ?>&bed=<?php echo $bed; ?>"><?php printf(_x('Select %s', 'Select bed/room button', 'quitenicebooking'), $defs['description']); ?></button>
											<?php } else { ?>
												<span class="button2 btn btn-warning"><?php printf(_x('Select %s', 'Select bed/room button', 'quitenicebooking'), $defs['description']); ?></span>
											<?php } ?>
											<?php $bed_type_count ++; ?>
										<?php } // end if ?>
									<?php } // end foreach ?>
									...
									<?php if ($bed_type_count == 0) { // display "Select Room" button ?>
										<?php if ( $highlight_found ) { ?>
											<button type="submit" class="button2 btn btn-warning" name="room_<?php echo $quitenicebooking_room_number; ?>_selection" value="type=<?php echo $room['id']; ?>&bed=0"><?php printf(_x('Select %s', 'Select bed/room button', 'quitenicebooking'), $quitenicebooking_beds->keys['disabled']['description']); ?></button>
										<?php } else { ?>
											<span class="button2 btn btn-warning"><?php printf(_x('Select %s', 'Select bed/room button', 'quitenicebooking'), $quitenicebooking_beds->keys['disabled']['description']); ?></span>
										<?php } ?>
									<?php } // end if ?>
									<!-- END .bookroom -->
							...
							<?php foreach ($quitenicebooking_available_rooms as $room) { ?>
								...
								<li class="room-item clearfix">
									...
									<div class="room-list-right">
										...
										<?php if ($bed_type_count == 0) { // display "Select Room" button ?>
											<button type="submit" class="button2 btn-warning" name="room_<?php echo $quitenicebooking_room_number; ?>_selection" value="type=<?php echo $room['id']; ?>&bed=0"><?php printf(_x('Select %s', 'Select bed/room button', 'quitenicebooking'), $quitenicebooking_beds->keys['disabled']['description']); ?></button>
										<?php } // end if ?>
										<!-- END .bookroom -->
